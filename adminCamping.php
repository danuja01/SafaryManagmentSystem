...
if(isset($_POST['delete'])){
    ...
    $check = mysqli_query($conn,"SELECT * FROM camping WHERE id = '$key' ") or die("no id");

    if(mysqli_num_rows($check)>0){
        //if records found and deleted

        $delete = mysqli_query($conn,"DELETE FROM camping WHERE id = '$key'") or die("NOT DELETED");
        header('location:adminCamping.php');
...
<div class="table">
    <table id="customers">
        ...
        <tr>
            <form action="adminCamping.php" method="POST" role="form">
                <td><?php echo htmlspecialchars($bin['id']); ?></td>
                <td><?php echo htmlspecialchars($bin['category']); ?></td>
